    ...
    public static void main(String[] args) throws Exception {
        ...
        try (IRODSConnection conn = new IRODSConnection()) {
            ...
            for (int run = 1; run <= TEST_RUNS; run++) {
                for (File file : files) {
                    runSingleCompressionTest(rcComm, file, run, user, zone);
                }
            }
        }
    }

    private static void runSingleCompressionTest(RcComm rcComm, File file, int run, String user, String zone) throws Exception {
        ...
        System.out.printf("Compressed %s (%.2f MB → %.2f MB)%n",
                file.getName(), file.length() / 1e6, compressedSize / 1e6);

        // Upload
        DataObjInp_PI createInp = new DataObjInp_PI();
        createInp.objPath = irodsPath;
        createInp.dataSize = compressedSize;
        createInp.oprType = 0;
        ...
        System.out.printf("Run %d | Uploaded (compressed) %s in %.2fs (%.2f MB/s)%n",
        ...
        int readFd = IRODSApi.rcDataObjCreate(rcComm, openInp);
        ...
        double downloadRate = (compressedSize / 1e6) / downloadTime;
        System.out.printf("Run %d | Downloaded (compressed) %s in %.2fs (%.2f MB/s)%n",

...
/**
 * iRODS Performance Test - Safe for large files up to 1GB
 */
public class IrodsPerformanceTest {

    private static final int TEST_RUNS = 3;
    private static final int MAX_THREADS = 4; // adjust based on your CPU/IO
    private static final int BUFFER_SIZE = 128 * 1024; // 128 KB buffer
    private static final boolean ENABLE_COMPRESSION = false; // disable for large files
    ...
    public static void main(String[] args) {
        log(GREEN, "=".repeat(80));
        log(GREEN, "iRODS Performance Test - Large File Safe");
        log(GREEN, "=".repeat(80));
    ...
    private static List<Result> runTests(RcComm rcComm, Config config, List<File> testFiles) throws InterruptedException, ExecutionException {
        List<Result> results = new ArrayList<>();
        ExecutorService executor = Executors.newFixedThreadPool(MAX_THREADS);

        log(YELLOW, String.format("\nRunning %d test iterations with up to %d parallel threads...", TEST_RUNS, MAX_THREADS));

        for (int run = 1; run <= TEST_RUNS; run++) {
            final int runIndex = run; // fix lambda capture
            List<Future<Result>> futures = new ArrayList<>();

            for (File testFile : testFiles) {
                Future<Result> future = executor.submit(() -> performTest(rcComm, config, testFile, runIndex));
                futures.add(future);
            }
            ...
            for (Future<Result> f : futures) {
                try {
                    ...
                    log(GREEN, String.format("  Run %d/%d [%s]: Upload %.3fs, Download %.3fs",
                            r.run, TEST_RUNS, r.filename, r.uploadTime, r.downloadTime));
                } catch (ExecutionException ee) {
                    log(RED, "  Test FAILED: " + ee.getCause().getMessage());
                }
            }
        }
    ...
    private static Result performTest(RcComm rcComm, Config config, File testFile, int run) throws Exception {
        ...
        if (ENABLE_COMPRESSION && originalSize < 100 * 1024 * 1024) { // only compress small files
        ...
        return new Result(run, filename, originalSize, uploadFile.length(), uploadTime, downloadTime, compressTime, decompressTime);
    }
